...
@Component({
  ...
  template: `
    <div class="space-y-8">
      ...
      <!-- Create Key Form -->
      <div *ngIf="showCreateForm()" class="card border-2 border-primary-200 dark:border-primary-800">
      ...
      <!-- Success Message -->
      <div *ngIf="showSuccessMessage()" class="bg-success-50 border border-success-200 text-success-700 px-4 py-3 rounded-lg dark:bg-success-900/20 dark:border-success-800 dark:text-success-200">
        <p class="font-medium">{{ successMessage() }}</p>
      </div>

      <!-- API Keys List -->
      <div class="space-y-4">
        <div *ngFor="let apiKey of apiKeys()" class="card hover:shadow-md transition-shadow">
          <div class="card-body">
            ...
            <div class="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-4 font-mono text-sm mb-4 flex items-center justify-between">
              <code *ngIf="!apiKey.revealed" class="text-neutral-600 dark:text-neutral-400">
                {{ apiKey.maskedKey }}
              </code>
              <code *ngIf="apiKey.revealed" class="text-success-600 dark:text-success-400 break-all">
                {{ apiKey.key }}
              </code>
              <button
                type="button"
                *ngIf="!apiKey.revealed"
            ...
            <div class="mb-4">
              ...
              <div class="flex gap-2 flex-wrap">
                <span *ngFor="let scope of apiKey.scopes" class="badge-primary">
                  {{ scope }}
                </span>
              </div>
            </div>
            ...
            <!-- Actions -->
            <div *ngIf="apiKey.status === 'active'" class="flex gap-2">
              <button
                type="button"
        ...
        <!-- Empty State -->
        <div *ngIf="apiKeys().length === 0 && !showCreateForm()" class="card text-center py-12">
